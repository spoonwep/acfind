...
<head>
<meta charset="utf-8" />
<title>AC Find——查找A站源视频、下载视频、弹幕</title>
<style>
#nav_left {
    height: 510px;
    width: 90px;
...
if(@$_POST['url']){
	  ...
	  $bb=is_ac($url);
	  if(!$bb){
		  echo '<script>alert("这好像不是A站的地址吧少年哟");</script>
		  <center><img src="image/error.png" style="height:646px;width:438px;"></center>
		  		';
		  }else
		  {  
...
if(isset($type[1])||isset($redirect_url_info['vtype'])){
	...
	if($redirect_url_info['vtype']=='qq'){
	 ...
	 echo '
				...
				<form name="form1" method="post" action="">
					...
					<div id="nav_right" style="margin-top: 0px;" onclick="go_next();">
						<div class="nav_mark_right">》</div>
					</div>
				</form>';
	 echo '
	 	<div class="acfun">
			<object id="ac" style="display: block;margin: 0 auto;margin-top: -524px;margin-top: 20px;box-shadow: 0 0 15px black;-webkit-box-shadow: 0 0 15px black;-moz-box-shadow: 0 0 15px black;-o-box-shadow: 0 0 15px black;" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="860" height="524">
				<param name="wmode" value="transparent" />
				<param name="movie" value="'.$final.'" />
				<param name="allowFullScreen" value="true" />
        		<!--[if !IE]>-->
				<object id="ac1" type="application/x-shockwave-flash" data="'.$final.'" width="860" height="524">
				<!--<![endif]-->
				<div>
					<h1>Acfind</h1>
				</div>
			...
			';
	echo '<a href="http://acfind.sinaapp.com" title="点击视频信息区域可快速返回首页" class="videoinfo"><div class="decription" >介绍:'.$up_decription.'<div id="aai">
					<span class="item">收藏用户：<span class="pts">'.$up_stows.'</span></span>
					<span class="item">播放次数：<span class="pts">'.$up_clicks.'</span></span>
			  </div></div></a>
			  </div>';

	echo '<div class="userinfo"><a href="http://www.acfun.tv/member/user.aspx?uid='.$up_id.'" target="_blank" ><img src="'.$up_ava.'" class="yooo" title="我是UP主'.$up_name.'，少年准备来一发吗？" style="width:100px"></a></div>';
	echo '<div class="download" title="功能待添加"></div>';
	$cc=download($url);
		echo '
		...
		<span>
			...
			<form method="GET"   action="http://images.google.com.hk/searchbyimage" style="display:inline;" target="_blank">
				...
				<button type="submit" name="" class="btn">谷歌搜封面</button>
				<input type="hidden" name="newwindow" value="1">
			</form>
			<button class="btn" id="zoom_up" >网页全屏</button>
			<button class="btn" id="zoom_down" >关闭全屏</button>
		</span>
		</div>
		...
		<div id="full-width" class="modal container hide fade" tabindex="-1">
			...
			<div class="modal-body" >
				...
				<a id="get_lst" class="btn btn-large btn-primary" href="./download/'.$baseurl.'.lst" target="_blank">导出下载列表</a>
				</div>
			</div>
        ...
        <input class="btn btn-large btn-primary" style="margin-top: 11px;position: absolute;margin-left:200px;" type="submit" value="下载ASS外挂字幕" target="_balnk">
    </form>
	...
	if(@$type[1]=="video.sina"||@$type[1]=='you.video.sina'){
		...
		<a href="'.$url.'" target="_blank" title="点我回到主站对应视频" class="backtoac">'.$up_title.'</a></div>';
		if(!empty($prev)){
			echo '
				<form name="form" method="post" action="">
					<input type="hidden" value="'.$prev.'" name="url"/>
	 ...
	 echo '<div class="acfun">
			<object id="ac" name="obj1" style="display: block;margin: 0 auto;margin-top: -524px;margin-top: 20px;box-shadow: 0 0 15px black;-webkit-box-shadow: 0 0 15px black;-moz-box-shadow: 0 0 15px black;-o-box-shadow: 0 0 15px black;" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="860" height="510">
				<param name="movie" value="http://you.video.sina.com.cn/api/sinawebApi/outplayrefer.php/vid='.$final.'/s.swf" />
				<param name="allowFullScreen" value="true" />
        		<!--[if !IE]>-->
				<object id="ac1" type="application/x-shockwave-flash" data="http://you.video.sina.com.cn/api/sinawebApi/outplayrefer.php/vid='.$final.'/s.swf" width="860" height="510">
				<!--<![endif]-->
				<div>
					<h1>Acfind</h1>
				</div>
	...
	$cc=download($url);
		...
		<span>
			...
			<form method="GET"   action="http://images.google.com.hk/searchbyimage" style="display:inline;" target="_blank">
				<input  type="hidden" name="image_url"  value="'.@$redirect_url_info['vinfo']['image'].'">
				<button type="submit" name="" class="btn" id="gugeniang" >谷歌搜封面</button>
				<input type="hidden" name="newwindow" value="1">
			</form>
			<button class="btn" id="zoom_up" >网页全屏</button>
			<button class="btn" id="zoom_down" >关闭全屏</button>
	...
	if(@$type[1]=="www.tudou"){
	 ...
	 echo '
	 	<div class="acfun">
			<iframe id="ac" frameborder=0 style="display: block;margin: 0 auto;margin-top: -524px;margin-top: 20px;padding-left: 90px;left: 20px;" height="524" width="860" src="http://www.tudou.com/programs/view/html5embed.action?code='.$final.'"></iframe>
			';
	echo '<a href="http://acfind.sinaapp.com" title="点击视频信息区域可快速返回首页" class="videoinfo"><div class="decription" >介绍:'.$up_decription.'<div id="aai">
					<span class="item">收藏用户：<span class="pts">'.$up_stows.'</span></span>
					<span class="item">播放次数：<span class="pts">'.$up_clicks.'</span></span>
...
if(@$type[1]=="v.youku"){
	 ...
	 echo '
	 	<div class="acfun">
	 		<object id="ac" name="obj1" style="display: block;margin: 0 auto;margin-top: -524px;margin-top: 20px;box-shadow: 0 0 15px black;-webkit-box-shadow: 0 0 15px black;-moz-box-shadow: 0 0 15px black;-o-box-shadow: 0 0 15px black;" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="860" height="510">
				<param name="movie" value="http://static.youku.com/v1.0.0362/v/swf/loader.swf?VideoIDS='.$final.'&winType=adshow&isAutoPlay=false" />
				<param name="allowFullScreen" value="true" />
        		<!--[if !IE]>-->
				<object id="ac1" type="application/x-shockwave-flash" data="http://static.youku.com/v1.0.0362/v/swf/loader.swf?VideoIDS='.$final.'&winType=adshow&isAutoPlay=false" width="860" height="510">
				<!--<![endif]-->
				<div>
					<h1>Acfind</h1>
				</div>
    ...
    <a href="http://comment.acfun.tv/'.$redirect_url_info['cid'].'.json" class="btn btn-large btn-primary" title="此格式弹幕与上方参数配置无关">下载Json格式弹幕</a>
		...
		echo '无法解析视频!可能是主站视频不存在，或者是史前巨坟。';
	  }
}
}else{
	echo '
	...
	<div class="form">
			<form action="index.php" method="post">
				<input type="text" name="url" class="txts" onmouseover="this.select()" title="请输入A站视频地址,带http://哦输错的话会发生不得了的事情的哦，输入完成请点击下方的一发入魂" placeholder="请在此输入喵~"/>'.'<br><br>'.'
...
<link type="text/css" rel="stylesheet" href="css/search.css" />
	<script type="text/javascript">
		...
		$("#zoom_up").click(function(){
			$("#ac").css("width",$(document.body).width());
			$("#ac").css("height",$(document.body).height());
			$("#ac1").css("width",$(document.body).width());
			$("#ac1").css("height",$(document.body).height());
			$('#ac').css("position","absolute");
			$('#ac1').css("position","absolute");
			$('.titles').css("opacity",'0');
			$('.videoinfo').css("opacity",'0');
			$('.show_dl_menu').css("position","absolute");
			$('.show_dl_menu').css("top",'0');
		...
		$("#zoom_down").click(function(){
			...
			$("#ac").css("height",'510px');
			$("#ac1").css("width",'860px');
			$("#ac1").css("height",'510px');
			$('#ac').css("position","static");
			$('#ac1').css("position","static");
			$('.titles').css("opacity",'1');
			$('.videoinfo').css("opacity",'1');
			$('.show_dl_menu').css("position","static");
			$('.show_dl_menu').css("top",'auto');
			});
	...
	<div id="footer"> Copyright 2013 By 橙橙@<a href="http://lolimilk.com" title="我的个人博客" class="footer" target="_balnk">lolimilk</a> Email:admin#lolimilk.com</div>
